...
- Name tests after the scenario they express, not the function they call.
  Good: `test('user can cancel an order before it ships')`
  Bad: `test('cancelOrder')`
...
Run the test suite and capture the output. The right command depends on where
the change lives:

```bash
# Top-level E2E (Playwright)
rtk npx playwright test 2>&1 | tail -50

# Client (in client/)
cd client && rtk npm test 2>&1 | tail -50

# Server (in server/)
cd server && rtk npm test 2>&1 | tail -50
```
