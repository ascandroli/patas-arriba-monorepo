...
```bash
# Pick the commands that match where the change lives:
cd client && rtk npm run lint     # client lint
cd client && rtk npm test         # client tests
cd server && rtk npm test         # server tests
rtk npx playwright test           # top-level E2E
```
